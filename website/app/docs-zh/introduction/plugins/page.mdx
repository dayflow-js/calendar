# **插件系统**

Day Flow 采用插件架构来扩展功能。插件提供模块化特性，您可以根据需要启用、禁用或配置这些功能。

## **插件系统概览**

插件系统允许您：

- **启用/禁用功能** - 按需加载所需功能
- **配置行为** - 自定义插件设置
- **扩展功能** - 创建自定义插件
- **访问插件 API** - 在代码中使用插件服务

## **内置插件**

Day Flow 提供两个核心插件：

### **拖拽插件**

支持通过拖拽创建、移动和调整事件时长。

**功能特性：**

- 拖拽移动事件
- 调整大小改变事件时长
- 双击创建新事件
- 在全天区域拖拽创建全天事件
- 视图特定行为

**安装方式：**

```tsx
import {
  useCalendarApp,
  DayFlowCalendar,
  createMonthView,
  ViewType,
} from '@dayflow/core';
import { createDragPlugin } from '@dayflow/plugin-drag';
import '@dayflow/core/dist/styles.css';

function MyCalendar() {
  const dragPlugin = createDragPlugin({
    enableDrag: true,
    enableResize: true,
    enableCreate: true,
    enableAllDayCreate: true,
    supportedViews: [ViewType.DAY, ViewType.WEEK, ViewType.MONTH],
  });

  const calendar = useCalendarApp({
    views: [createMonthView()],
    plugins: [dragPlugin],
  });

  return <DayFlowCalendar calendar={calendar} />;
}
```

**配置选项：**

```tsx
interface DragPluginConfig {
  enableDrag: boolean;// 启用事件拖拽（默认：true）
  enableResize: boolean;// 启用调整事件大小（默认：true）
  enableCreate: boolean;// 启用双击创建事件（默认：true）
  enableAllDayCreate: boolean;// 启用创建全天事件（默认：true）
  supportedViews: ViewType[];// 支持拖拽的视图（默认：[DAY, WEEK, MONTH]）}
```

**默认配置：**

```tsx
const dragPlugin = createDragPlugin({
  enableDrag: true,
  enableResize: true,
  enableCreate: true,
  enableAllDayCreate: true,
  supportedViews: [ViewType.DAY, ViewType.WEEK, ViewType.MONTH],
});
```

**访问拖拽 API：**

```tsx
// 从日历应用获取拖拽服务
const dragService = calendar.app.getPlugin<DragService>('drag');

// 获取当前配置
const config = dragService?.getConfig();

// 动态更新配置
dragService?.updateConfig({
  enableResize: false,
});

// 检查视图是否支持拖拽
const isSupported = dragService?.isViewSupported(ViewType.MONTH);
```

### **事件插件**

提供高级事件管理能力，包括验证、筛选和查询功能。

**功能特性：**

- 事件验证
- 日期范围查询
- 事件筛选
- 自动日期重计算
- 每日事件数量限制

**安装方式：**

```tsx
import {
  useCalendarApp,
  DayFlowCalendar,
  createEventsPlugin,
  createMonthView,
} from '@dayflow/core';
import '@dayflow/core/dist/styles.css';

function MyCalendar() {
  const eventsPlugin = createEventsPlugin({
    enableAutoRecalculate: true,
    enableValidation: true,
    maxEventsPerDay: 50,
  });

  const calendar = useCalendarApp({
    views: [createMonthView()],
    plugins: [eventsPlugin],
  });

  return <DayFlowCalendar calendar={calendar} />;
}
```

**配置选项：**

```tsx
interface EventsPluginConfig {
  enableAutoRecalculate?: boolean; // 自动重计算事件日期（默认：true）
  enableValidation?: boolean; // 添加/更新前验证事件（默认：true）
  defaultEvents?: Event[]; // 初始事件（默认：[]）
  maxEventsPerDay?: number; // 每日最大事件数（默认：50）
}
```

**访问事件 API：**

```tsx
// 从日历应用获取事件服务
const eventsService = calendar.app.getPlugin<EventsService>('events');

// 获取所有事件
const allEvents = eventsService?.getAll();

// 根据 ID 获取事件
const event = eventsService?.getById('event-1');

// 根据日期获取事件
const todayEvents = eventsService?.getByDate(new Date());

// 根据日期范围获取事件
const rangeEvents = eventsService?.getByDateRange(
  new Date(2024, 9, 1),
  new Date(2024, 9, 31)
);

// 获取特定星期几的事件
const mondayEvents = eventsService?.getByDay(0, weekStart);

// 获取全天事件
const allDayEvents = eventsService?.getAllDayEvents(0, events);

// 验证事件
const errors = eventsService?.validateEvent(event);
if (errors && errors.length > 0) {
  console.error('验证错误:', errors);
}

// 根据日历类型筛选事件
const filteredEvents = eventsService?.filterEvents(
  events,
  event => event.calendarId === 'work'
);

// 重计算事件日期
const recalculatedEvents = eventsService?.recalculateEventDays(
  events,
  weekStart
);
```

## **使用多个插件**

您可以组合多个插件以获得完整功能：

```tsx
import {
  useCalendarApp,
  DayFlowCalendar,
  createEventsPlugin,
  createMonthView,
  createWeekView,
} from '@dayflow/core';
import { createDragPlugin } from '@dayflow/plugin-drag';
import '@dayflow/core/dist/styles.css';

function MyCalendar() {
  const dragPlugin = createDragPlugin({
    enableDrag: true,
    enableResize: true,
    enableCreate: true,
  });

  const eventsPlugin = createEventsPlugin({
    enableValidation: true,
    maxEventsPerDay: 50,
  });

  const calendar = useCalendarApp({
    views: [createMonthView(), createWeekView()],
    plugins: [dragPlugin, eventsPlugin],
    events: initialEvents,
  });

  return <DayFlowCalendar calendar={calendar} />;
}
```

## **同时使用两个插件**

在大多数使用场景中，您会希望同时使用这两个插件：

```tsx
import {
  useCalendarApp,
  DayFlowCalendar,
  createEventsPlugin,
  createMonthView,
} from '@dayflow/core';
import { createDragPlugin } from '@dayflow/plugin-drag';
import '@dayflow/core/dist/styles.css';

function MyCalendar() {
  const dragPlugin = createDragPlugin({
    enableDrag: true,
    enableResize: true,
  });

  const eventsPlugin = createEventsPlugin({
    enableValidation: true,
    maxEventsPerDay: 100,
  });

  const calendar = useCalendarApp({
    views: [createMonthView()],
    plugins: [dragPlugin, eventsPlugin],
  });

  return <DayFlowCalendar calendar={calendar} />;
}
```

## **创建自定义插件**

您可以创建自己的插件来扩展日历功能：

### **插件接口**

```tsx
interface CalendarPlugin {
  name: string;// 唯一插件名称install: (app: CalendarApp) => void;// 安装函数
  config?: Record<string, unknown>;// 插件配置
  api?: unknown;// 公共 API}
```

### **示例：数据分析插件**

```tsx
import { CalendarPlugin, CalendarApp, Event } from '@dayflow/core';

interface AnalyticsConfig {
  trackEventCreate: boolean;
  trackEventUpdate: boolean;
  trackEventDelete: boolean;
  trackViewChange: boolean;
}

interface AnalyticsService {
  logEvent: (name: string, data: any) => void;
  getStats: () => { eventCreates: number; eventUpdates: number };
}

export function createAnalyticsPlugin(
  config: Partial<AnalyticsConfig> = {}
): CalendarPlugin {
  const finalConfig: AnalyticsConfig = {
    trackEventCreate: true,
    trackEventUpdate: true,
    trackEventDelete: true,
    trackViewChange: true,
    ...config,
  };

  let stats = {
    eventCreates: 0,
    eventUpdates: 0,
    eventDeletes: 0,
    viewChanges: 0,
  };

  const analyticsService: AnalyticsService = {
    logEvent: (name: string, data: any) => {
      console.log(`[数据分析] ${name}:`, data);
      // 发送到您的数据分析服务
    },
    getStats: () => ({
      eventCreates: stats.eventCreates,
      eventUpdates: stats.eventUpdates,
    }),
  };

  return {
    name: 'analytics',
    config: finalConfig,
    install: (app: CalendarApp) => {
      console.log('📊 数据分析插件已安装');

      // 挂载到日历回调函数
      const originalAddEvent = app.addEvent.bind(app);
      app.addEvent = (event: Event) => {
        if (finalConfig.trackEventCreate) {
          stats.eventCreates++;
          analyticsService.logEvent('event_created', { id: event.id });
        }
        originalAddEvent(event);
      };

      const originalUpdateEvent = app.updateEvent.bind(app);
      app.updateEvent = (id: string, updates: Partial<Event>) => {
        if (finalConfig.trackEventUpdate) {
          stats.eventUpdates++;
          analyticsService.logEvent('event_updated', { id });
        }
        originalUpdateEvent(id, updates);
      };

      const originalDeleteEvent = app.deleteEvent.bind(app);
      app.deleteEvent = (id: string) => {
        if (finalConfig.trackEventDelete) {
          stats.eventDeletes++;
          analyticsService.logEvent('event_deleted', { id });
        }
        originalDeleteEvent(id);
      };
    },
    api: analyticsService,
  };
}
```

### **使用自定义插件**

```tsx
import {
  useCalendarApp,
  DayFlowCalendar,
  createMonthView,
} from '@dayflow/core';

function MyCalendar() {
  const analyticsPlugin = createAnalyticsPlugin({
    trackEventCreate: true,
    trackEventUpdate: true,
  });

  const calendar = useCalendarApp({
    views: [createMonthView()],
    plugins: [analyticsPlugin],
  });

  // 访问插件 APIconst analytics = calendar.app.getPlugin<AnalyticsService>('analytics');
  const stats = analytics?.getStats();

  return <DayFlowCalendar calendar={calendar} />;
}
```

## **插件最佳实践**

### **1. 命名规范**

为插件使用描述性且唯一的名称：

```tsx
// 推荐
name: 'analytics';
name: 'custom-validation';
name: 'sync';

// 不推荐
name: 'plugin';
name: 'p1';
```

### **2. 配置设计**

提供合理的默认值，并使配置可选：

```tsx
const defaultConfig = {
  enabled: true,
  timeout: 5000,
};

export function createMyPlugin(config: Partial<MyConfig> = {}) {
  const finalConfig = { ...defaultConfig, ...config };
  // ...
}
```

### **3. API 设计**

为插件功能提供清晰、类型化的 API：

```tsx
interface MyPluginService {
  doSomething: () => void;
  getSomething: () => SomeData;
}

const service: MyPluginService = {
  doSomething: () => {
    /* ... */
  },
  getSomething: () => {
    /* ... */
  },
};

return {
  name: 'my-plugin',
  api: service,
};
```

## **插件生命周期**

1. **插件创建** - 使用配置调用 `createMyPlugin()`
2. **插件注册** - 添加到 `useCalendarApp` 的 `plugins` 数组中
3. **安装** - 使用 CalendarApp 实例调用 `install()` 函数
4. **使用** - 通过 `app.getPlugin()` 访问插件 API

```tsx
// 1. 创建
const plugin = createDragPlugin({ enableDrag: true });

// 2. 注册
const calendar = useCalendarApp({
  plugins: [plugin], // 在此处进行安装
});

// 3. 使用
const dragService = calendar.app.getPlugin<DragService>('drag');
```

## **检查插件是否存在**

在使用插件前始终检查插件是否已安装：

```tsx
function MyComponent({ calendar }: { calendar: CalendarApp }) {
  // 检查插件是否存在
  if (calendar.hasPlugin('drag')) {
    const dragService = calendar.getPlugin<DragService>('drag');
    // 使用拖拽服务
  }

  // 或者获取并检查
  const eventsService = calendar.getPlugin<EventsService>('events');
  if (eventsService) {
    const events = eventsService.getAll();
  }
}
```

## **插件 TypeScript 类型**

```tsx
import type {
  CalendarPlugin,
  DragService,
  DragPluginConfig,
  EventsService,
  EventsPluginConfig,
} from '@dayflow/core';

// 类型安全的插件使用const dragService = app.getPlugin<DragService>('drag');
const eventsService = app.getPlugin<EventsService>('events');
```

## **相关文档**

- [快速开始](docs/introduction) - 基础设置
- [事件管理](docs/events) - 处理日历事件
- [视图管理](docs/views) - 理解日历视图
- [API 参考](docs/api-reference) - 完整的 API 文档
