...
| `locale`               | `string \| Locale`         | ❌   | `'en-US'`           | 设置国际化（i18n）的语言环境。支持语言代码（如 'zh'）或 Locale 对象。 |
| `useSidebar`           | `boolean \| SidebarConfig` | ❌   | `false`             | 启用内置侧边栏或让您自定义宽度、折叠状态和渲染器                      |
| `useEventDetailDialog` | `boolean`                  | ❌   | `false`             | 启用默认模态详情对话框而不是内联面板                                  |
| `useCalendarHeader`    | `boolean \| Function`      | ❌   | `true`              | 启用默认页眉、隐藏页眉（`false`）或提供自定义渲染函数                 |
| `readOnly`             | `boolean \| ReadOnlyConfig`| ❌   | `false`             | 禁用交互。可以是布尔值，也可以是用于细粒度控制（拖拽/查看）的配置对象 |
| `customMobileEventRenderer` | `MobileEventRenderer` | ❌   | —                   | 用于替换默认移动端事件抽屉/对话框的自定义组件                         |
...
- `true` (默认)：渲染内置的日历页眉。
- `false`：完全隐藏页眉。
- `(props) => ReactNode`：渲染自定义页眉。详见 [日历页眉](/docs-zh/features/calendar-header)。

### readOnly

- `true`：禁用所有修改（拖拽、创建、编辑）。
...
- 详见 [移动端支持](/docs-zh/features/mobile-support)。
