...
サイドパネルではなくモーダルでイベント詳細を見せたい場合は、`customEventDetailDialog` prop に任意のコンポーネントを渡します。DayFlow 側が開閉タイミングを制御し、あなたは表現や入力フォームに集中できます。

<CustomDetailDialogShowcase />

## 押さえておきたいポイント

- 渡されるプロップスは `event`, `isOpen`, `onClose`, `onEventUpdate`, `onEventDelete` の 5 つ。
- JSX であればどんな構造でも OK。既存のモーダルコンポーネントにそのまま組み込めます。
- ダイアログの開閉はカレンダーが管理しています。処理が完了したら `onClose()` を呼び出してください。
- `onEventUpdate` と `onEventDelete` はフック済みの CRUD アクションなので、フォーム送信や削除ボタンから直接呼べます。
...
const CustomDialog: EventDetailDialogRenderer = ({
  ...
  const handleColor = (next: string) => onEventUpdate({ ...event, color: next });
    ...
    >
      <div className="relative w-full max-w-3xl overflow-hidden rounded-3xl bg-white shadow-2xl ring-1 ring-black/5">
        ...
        <header className="relative px-8 pt-10 pb-6 text-white">
          ...
          {event.description && (
            <p className="mt-3 max-w-xl text-sm text-white/80">{event.description}</p>
          )}
        </header>
        ...
        <section className="relative grid gap-6 px-8 pb-8 sm:grid-cols-[1.6fr,1fr]">
          <article className="rounded-2xl border border-gray-100 bg-white/70 p-5 shadow-sm backdrop-blur">
            ...
            <p className="mt-3 text-sm text-gray-700">
              {event.allDay ? 'All day' : `${event.start.toString()} → ${event.end?.toString()}`}
            </p>
          </article>
          ...
          <aside className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-xs text-gray-500">ブランドカラーや優先度の色に合わせられます。</p>
...
このようにモーダルの見た目を自由にコントロールしつつ、DayFlow のライフサイクルと整合性を保てます。
