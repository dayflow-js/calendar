...
| `locale`               | `string \| Locale`         | ❌   | `'en-US'`            | 国際化（i18n）のためのロケール設定。言語コード（'ja'など）またはLocaleオブジェクト。 |
...
| `useCalendarHeader`    | `boolean \| Function`      | ❌   | `true`               | デフォルトヘッダーの有効化、非表示（`false`）、またはカスタムレンダラーの指定。      |
| `readOnly`             | `boolean \| ReadOnlyConfig`| ❌   | `false`              | インタラクションを無効化。真偽値、または詳細制御（ドラッグ/表示）用の設定オブジェクト。|
| `customMobileEventRenderer` | `MobileEventRenderer` | ❌   | —                    | デフォルトのモバイル用イベントドロワー/ダイアログを置き換えるカスタムコンポーネント。 |
...
- `true` (デフォルト): 標準のカレンダーヘッダーをレンダリングします。
- `false`: ヘッダーを完全に非表示にします。
- `(props) => ReactNode`: カスタムヘッダーをレンダリングします。詳細は [カレンダーヘッダー](/docs-ja/features/calendar-header) を参照してください。

### readOnly

- `true`: すべての変更（ドラッグ、作成、編集）を無効にします。
- `ReadOnlyConfig`: きめ細かな制御。
  - `draggable`: ドラッグが可能かどうか。
  - `viewable`: イベント詳細を開くことが可能かどうか。
...
- 詳細は [モバイル対応](/docs-ja/features/mobile-support) を参照してください。
