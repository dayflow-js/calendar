...
| `locale`               | `string \| Locale`         | ❌       | `'en-US'`              | Sets the locale for internationalization (i18n). Supports language codes (e.g., 'ja') or Locale objects.           |
...
| `useCalendarHeader`    | `boolean \| Function`      | ❌       | `true`                 | Enables the default header, hides it (`false`), or lets you provide a custom renderer.                             |
| `readOnly`             | `boolean \| ReadOnlyConfig`| ❌       | `false`                | Disables interactions. Can be a boolean or a config for fine-grained control (drag/view).                          |
| `customMobileEventRenderer` | `MobileEventRenderer` | ❌       | —                      | Custom component to replace the default mobile event drawer/dialog.                                                |
...
- See [Features Overview](/docs/features/sidebar) for more on specific features.
