...
| `color`       | `string`                                                           | Display color as hex (`#3b82f6`) or CSS color name                                                             | ❌ No    |
| `meta`        | `Record<string, any>`                                              | Additional custom metadata (location, attendees, etc.)                                                         | ❌ No    |
...
```tsx
import { createEvent, createAllDayEvent, createTimedEvent } from 'dayflow';
...
// Quick timed event (using decimal hours: 14.5 = 14:30)
...
```tsx
import { useCalendarApp, createMonthView, Event } from 'dayflow';
...
You can customize event appearance with colors:
...
const event: Event = {
  id: '1',
  title: 'Design Review',
  start: Temporal.ZonedDateTime.from('2024-10-15T14:00:00[America/New_York]'),
  end: Temporal.ZonedDateTime.from('2024-10-15T15:00:00[America/New_York]'),
  color: '#8b5cf6', // Purple
};
...
5. **Set appropriate colors** - Use consistent color schemes across your app
...
- [Temporal Migration Guide](/docs/temporal-migration) - Complete guide to Temporal API
- Event interface: `/src/types/event.ts`

## Related Documentation

- [Temporal Migration](/docs/temporal-migration) - Migrating from Date to Temporal API
