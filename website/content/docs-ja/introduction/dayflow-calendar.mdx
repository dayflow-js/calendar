...
| `useEventDetailPanel`   | `boolean`              | 任意 | `false` を渡すと組み込みのイベント詳細フローティングパネルを非表示にします。           |
...
3. **完全に無効化** — アプリ側のコールバックで独自のモーダルを管理している場合は、`DayFlowCalendar` に `useEventDetailPanel={false}` を渡すとフローティングパネルを完全に非表示にできます。
