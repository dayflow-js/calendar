...
> **Opt out instead**: if you want to suppress the built-in floating panel without replacing it with your own dialog, pass `useEventDetailPanel={false}` on `DayFlowCalendar` instead of using a slot.
