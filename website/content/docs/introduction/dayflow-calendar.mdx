...
| `useEventDetailPanel`   | `boolean`              | Optional | Set to `false` to suppress the built-in floating event detail panel entirely.                       |
...
3. **Opt out entirely** – set `useEventDetailPanel={false}` on `DayFlowCalendar` to suppress the floating panel when you have your own event modals driven by app callbacks.

```tsx
// Suppress the built-in panel — use your own modal triggered elsewhere
<DayFlowCalendar calendar={calendar} useEventDetailPanel={false} />
```
